...
- (void)viewDidLoad {
    ...
    self.calculatorView.historyTable.separatorStyle = UITableViewCellSeparatorStyleSingleLineEtched;
    // Do any additional setup after loading the view, typically from a nib.
    self.calculatorView.returnHome.hidden = 1;
    self.calculatorView.historyTable.hidden = 1;
    NSLog(@"Height: %d, Width: %d",self.calculatorView.frame.size.height,self.calculatorView.frame.size.width);
...
- (void)click:(UIButton *)button{
    ...
    if (button.tag == 117) {
        ...
        [self.calculatorView insertOneHistory:self.textView.text];
    ...
    if(button.tag == 118){
        //打开一个新的页面显示计算历史
        NSLog(@"goto newVC");
        [self.calculatorView bringSubviewToFront:self.calculatorView.historyTable];
        [self.calculatorView bringSubviewToFront:self.calculatorView.returnHome];
        self.calculatorView.returnHome.hidden = 0;
        self.calculatorView.historyTable.hidden = 0;
    }
}
